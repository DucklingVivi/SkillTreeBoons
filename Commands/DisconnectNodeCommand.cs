...
{
    public class DisconnectNodeCommand : ModCommand
    {
        ...
        public override string Usage
            => "/disconnect [node id][node id]";

        public override string Description
            => "Disconnects two nodes and writes the result to a file";
        ...
        public override void Action(CommandCaller caller, string input, string[] args)
        {
            ...
            foreach (JToken item in node1conns)
            {
                ...
                {
                    node1conns.Remove(item);
                    flag1 = true;
                    break;
                }
            }
            bool flag = !(flag0 || flag1);
            SkillTreeBoons.Instance.Logger.Info(flag);
            if (flag)
            {
                caller.Reply("Nodes already disconnected");
                return;
            }
